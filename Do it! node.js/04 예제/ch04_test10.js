...
// 타입을 확인합니다.
console.log('버퍼 객체의 타입 : %s', Buffer.isBuffer(buffer1)); // isBuffer() : 변수에 든게 버퍼 객체인지 아닌지 확인
...
buffer1.copy(buffer2, 0, 0, len);  // 다른 버퍼 객체로 복사할 때 사용
console.log('두 번째 버퍼에 복사한 후의 문자열 : %s', buffer2.toString('utf8'));

// 두 개의 버퍼를 붙여 줍니다.
var buffer3 = Buffer.concat([buffer1, buffer2]); // 두 개를 붙여서 새로운 버퍼 객체를 만들 때 사용
